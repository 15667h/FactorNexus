...
class ModelConfig:
    # ── 训练设备 ─────────────────────────────────────────────────────────
    # 历史结论（2026-07）：本任务 CPU 训练速度反而比 GPU 快（实测约 2.3 倍），
    # 原因：张量太小（2×3508×20）+ Python 串行调度，GPU 被切成碎片段。
    # 【P5 更新】该结论的前提是"小面板单品种"。升级到 A股多标的面板
    # （N≥100 股票 × F 特征）后张量足够大，GPU + torch.compile 收益显著，
    # 故 DEVICE 支持 auto 检测：AUTO_DEVICE=True 时优先 CUDA。
    # 单标的/小面板场景可设 AUTO_DEVICE=False 回退 CPU。
    AUTO_DEVICE: bool = False  # True=优先 CUDA；False=沿用历史 CPU 结论
    USE_TORCH_COMPILE: bool = False  # P5: torch.compile（面板化后建议开启）

    # 类体内直接内联求值（静态方法引用类名会 NameError）
    DEVICE = (
        torch.device("cuda")
