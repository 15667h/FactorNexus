...
def _train_predict(model, X_tr, y_tr, X_te):
...
def walk_forward_fit_predict(ds: FactorDataset, model_factory,
    ...
    Args:
        ...
        gap: 训练尾与预测首之间的间隔（防标签重叠泄漏，默认=horizon）。
    ...
    for fold_i, te_start in enumerate(starts):
        ...
        tr_end_ts = all_ts[te_start - 1] if te_start > 0 else None
